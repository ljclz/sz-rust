...
conn.on('ready', () => {
    ...
    conn.sftp((err, sftp) => {
        ...
        writeStream.on('close', () => {
            console.log('Upload done. Starting test in background...');
            const cmd = 'nohup bash -c \'cd /www/rust/sz-rust-enterprise && CARGO_TARGET_DIR=/www/rust/sz-rust-enterprise-target cargo test -p sz-rust-sz300 --test success_path_test -- --include-ignored --test-threads=1\' > /www/rust/test-fix.log 2>&1 & echo "PID: $!"';
