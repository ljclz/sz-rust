...
conn.on('ready', () => {
    const cmd = `kill -9 2705112 2>/dev/null; docker ps -a --filter "ancestor=mysql:9.6" -q 2>&1 | xargs -r docker rm -f 2>&1; cd /www/rust/sz-rust-enterprise && CARGO_TARGET_DIR=/www/rust/sz-rust-enterprise-target cargo llvm-cov -p sz-rust-sz300 --summary-only --ignore-filename-regex "main.rs" --lib ${testArgs} 2>&1 | tail -50`;
    console.log('Running coverage with all test targets...');
