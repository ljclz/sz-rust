...
#[tokio::main]
async fn main() {
    let port: u16 = std::env::var("PORT").ok().and_then(|s| s.parse().ok()).unwrap_or(8403);
    ...
    let app = Router::new()
        ...
        .route("/json", get(|| async { Json(json!({"message": "Hello, World!"})) }))
        .route("/user/{id}", get(|Path(id): Path<u64>| async move { Json(json!({"user_id": id})) }))
        .route("/db", get(db))
        .with_state(conn);
