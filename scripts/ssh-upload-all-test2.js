        ...
        files.forEach(f => {
            ...
            ws.on('close', () => {
                ...
                if (uploaded === files.length) {
                    ...
                    const cmd = 'docker ps -a --filter "ancestor=mysql:9.6" -q 2>&1 | xargs -r docker rm -f 2>&1; cd /www/rust/sz-rust-enterprise && CARGO_TARGET_DIR=/www/rust/sz-rust-enterprise-target cargo test -p sz-rust-sz300 --test success_path_test device_list_returns_owned_devices -- --include-ignored --test-threads=1 2>&1 | tail -25';
