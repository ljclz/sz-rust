...
|---------|-------------------------------|-----------|-----------|---------|------------|------|
| DB-2026-09-21-01 | OCR 冒烟缺陷修复提交（2026-09-21，见 CHANGELOG [Unreleased]-2026-09-21） | packages/sz-rust-mvc-facade/src/controller.rs + 上游 sz-orm-auth | **JWT audience 校验缺失（OCR 审查发现）**：`SZ_JWT_AUDIENCE` 此前被加载进 JwtConfig 但 `verify_token_with_config` 从不消费（no-op 假安全感）。本仓库侧已修复（移除 no-op 配置 + 文档如实标注"框架层无 audience 校验"）。**剩余上游依赖**：sz-orm-auth `JwtClaims`（7.6.0）无 `aud` 字段，真正的 aud 校验需上游在 JwtClaims 增加 `Option<String>` aud 字段（serde skip_serializing_if）后在 mvc-facade verify 链实现（iss 同款模式，controller.rs verify_token_with_config 第 5 步之后） | 2026-09-21 | 2026-10-21 | PENDING |
