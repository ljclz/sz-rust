...
4. **原子领取**：单条 `UPDATE ... WHERE id IN (SELECT ... LIMIT ?)` 抢占 + affected_rows 判定，不依赖 `FOR UPDATE SKIP LOCKED` 方言，MySQL/PostgreSQL 均兼容；多实例 worker 安全
...
- 已知取舍：v1 仅实现 MySQL 方言 SQL（sz300 主数据源），PG 支持待后续（领取 SQL 本身方言无关，建表 SQL 需调整）
