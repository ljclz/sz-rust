...
|---------|----------------|---------|
| HTTP 请求生命周期 | ✅ 必须记录 | ⏳ 待编写 |
| 路由匹配（前缀树 / 通配符 / 参数提取） | ✅ 必须记录 | ⏳ 待编写 |
| 中间件链（洋葱模型 / 执行顺序） | ✅ 必须记录 | ⏳ 待编写 |
| 控制器分发（trait + 默认方法） | ✅ 必须记录 | ⏳ 待编写 |
| Model 钩子（编译期注册表） | ✅ 必须记录 | ⏳ 待编写 |
| 请求体解析（serde + FromRequest） | ✅ 必须记录 | ⏳ 待编写 |
| 响应序列化（IntoResponse） | ✅ 必须记录 | ⏳ 待编写 |
| 认证授权（JWT / 中间件） | ✅ 必须记录 | ⏳ 待编写 |
| 事务操作（`#[transactional]`） | ✅ 必须记录 | ⏳ 待编写 |
| 缓存读写（Service 注入） | ✅ 必须记录 | ⏳ 待编写 |
...
以下 ADR 已识别为必须编写，但尚未完成（按优先级排序）：

| 优先级 | 编号 | 标题 | 触发原因 |
|--------|------|------|---------|
| P0 | ADR-001 | 路由策略选型（属性宏路由为主） | 影响全部 HTTP 请求分发 |
| P0 | ADR-002 | 中间件模型（Tower Service + 洋葱模型） | 影响全部横切关注点 |
| P0 | ADR-003 | 控制器抽象（trait + 默认方法 + 组合） | 影响全部业务代码组织 |
| P0 | ADR-004 | Model 钩子实现（编译期注册表） | 影响 PHP 迁移正确性 |
| P1 | ADR-005 | 事务管理策略（`#[transactional]` 宏） | 影响数据一致性 |
| P1 | ADR-006 | 认证授权机制（JWT + 中间件） | 影响安全性 |
| P1 | ADR-007 | addon 插件化机制（编译期注册） | 影响扩展性 |
| P1 | ADR-008 | 错误处理策略（`AppError` + Exception Filter） | 影响全部响应格式 |
| P2 | ADR-009 | 缓存 Service 注入策略 | 影响性能 |
| P2 | ADR-010 | 配置加载方式（serde + TOML） | 影响部署 |
