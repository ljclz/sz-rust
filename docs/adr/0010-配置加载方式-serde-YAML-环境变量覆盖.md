...
### 方案 A：继续使用 PHP 配置文件（拒绝）

```rust
// 在 Rust 中执行 PHP 配置文件（通过 embedded-php 或 FFI）
```

**拒绝原因**：
- Rust 无法直接执行 PHP 代码
- 需要嵌入 PHP 运行时，增加部署复杂度
- 失去 Rust 的类型安全优势（PHP 数组访问不存在的 key 返回 null，运行时错误）

### 方案 B：使用 JSON 格式（拒绝）

```json
{
  "app": { "default_app": "index" }
}
```

**拒绝原因**：
- JSON 不支持注释，配置文件难以维护
- JSON 不支持多行字符串，某些配置项（如邮件模板）难以表达
- YAML 是 Rust 生态中配置文件的事实标准（serde_yaml）
- PHP 迁移时 YAML 比 JSON 更接近 PHP 数组的直观表达

### 方案 C：使用 `config` crate（拒绝）

依赖 `config` crate（支持 YAML / JSON / TOML / 环境变量 / 远程配置的统一配置库）。

**拒绝原因**：
- `config` crate 的配置合并逻辑复杂，调试困难
- 无法精确控制环境变量覆盖的优先级和格式
- 自定义配置结构（5 个 section 对齐 PHP 5 个配置文件）更清晰
- `config` crate 的类型转换错误信息不友好，不利于生产调试
...
**拒绝原因**：
- PHP 端配置项有隐式默认值（数组访问不存在的 key 返回 null），Rust 端用默认值替代 null 更符合 PHP 行为
- 生产环境可能只覆盖部分配置（如只设置数据库密码），其他配置项应有合理默认值
- 默认值让配置加载更健壮，减少"配置项缺失"导致的服务启动失败

**最终选择**：serde + YAML + 环境变量覆盖 + 默认值。YAML 提供人类友好的配置格式，环境变量覆盖提供生产环境的敏感配置注入，默认值保证配置加载的健壮性。
...
4. **L4 代码层**：
   ...
   - **环境变量格式错误** → `SZ_APP_DEFAULT_APP=api`（单下划线）不会生效，必须是 `SZ_APP__DEFAULT_APP=api`（双下划线分隔层级）；检查 `config.rs` 的环境变量解析逻辑
   - **`.env` 文件未加载** → Rust 端需显式调用 `dotenvy::dotenv()` 加载 `.env` 文件，若忘记调用则环境变量来自 shell 环境，本地开发与生产行为不一致
