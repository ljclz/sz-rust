...
impl std::fmt::Debug for PayConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PayConfig")
            ...
            .field("merchant_private_key", &"<redacted>")
            .field("platform_public_key", &self.platform_public_key)
...
///
/// 与 `sz_rust_state_facade::notify::HttpTransport`（state-facade）不同，此 trait 的 `post_json` / `get`
...
mod tests {
    ...
    fn test_pay_config_builder() {
        ...
        // Debug 输出中商户私钥必须脱敏（安全铁律：敏感字段不泄漏）
        let debug_output = format!("{:?}", config);
        assert!(
            !debug_output.contains("MIIEvQIBADANB"),
            "P7-DES-01: Debug 输出泄漏商户私钥明文: {}",
            debug_output
        );
        assert!(
            debug_output.contains("<redacted>"),
            "P7-DES-01: Debug 输出应包含脱敏标记 <redacted>"
        );
