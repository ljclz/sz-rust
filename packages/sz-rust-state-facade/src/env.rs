...
"#;
    ...
    // ========================================================================
    // 并发边界测试（P3-CONC-01 ~ P3-CONC-04）
    // ========================================================================

    /// P3-CONC-01：Env 高并发读写一致性
    ///
    /// 验证 `Arc<RwLock<HashMap>>` 在高并发场景下：
    /// - 多读者并发读取不阻塞彼此
    /// - 写者与读者并发时数据不丢失
    /// - 最终一致性：所有写入最终可见
    #[tokio::test]
    async fn test_env_concurrent_read_write_consistency() {
        let env = Env::default();
        let num_writers = 8;
        let num_reads = 100;
        ...
        for i in 0..num_writers {
            env.set(&format!("key{}", i), &format!("initial{}", i));
        }
        ...
        for w in 0..num_writers {
            ...
            handles.push(tokio::spawn(async move {
                for r in 0..num_reads {
                    env.set(&format!("w{}_r{}", w, r), &format!("v{}_{}", w, r));
                }
            }));
        }
        ...
        for w in 0..num_writers {
            for r in 0..num_reads {
                let val = env.get(&format!("w{}_r{}", w, r));
                assert_eq!(
                    val,
                    Some(format!("v{}_{}", w, r)),
                    "P3-CONC-01: 写者 {} 记录 r={} 应可见",
                    w,
                    r
                );
            }
    ...
    /// P3-CONC-02：Env 无死锁 — 高并发 set/get 压力测试
    ///
    /// 使用超时包裹并发操作，验证不会发生死锁。
    /// 若 `parking_lot::RwLock` 在极端并发下出现死锁，
    /// `tokio::time::timeout` 将触发超时失败。
    #[tokio::test]
    async fn test_env_no_deadlock_under_contention() {
        let env = std::sync::Arc::new(Env::default());
        let num_tasks = 32;
            ...
            for t in 0..num_tasks {
                ...
                handles.push(tokio::spawn(async move {
                    for i in 0..ops_per_task {
                        let key = format!("k_{}_{}", t, i % 10);
                        if i % 3 == 0 {
                            env.set(&key, &format!("v_{}_{}", t, i));
                        } else {
                            let _ = env.get(&key);
                        }
                    }
        ...
        assert!(
            result.is_ok(),
            "P3-CONC-02: Env 高并发操作超时（可能存在死锁）"
        );
    }

    /// P3-CONC-03：parking_lot RwLock 不跨 .await 持有
    ///
    /// 验证 Env 的 get/set 方法不会在持有锁的状态下穿越 await 点。
    /// 测试方法：4 个任务通过 Barrier 同步后并发操作，
    /// 若锁跨 await 持有则会出现任务饥饿，timeout 将捕获。
    #[tokio::test]
    async fn test_env_lock_never_held_across_await() {
        let env = std::sync::Arc::new(Env::default());
        ...
        for t in 0..4 {
            ...
            handles.push(tokio::spawn(async move {
                barrier.wait().await; // 所有任务同步到同一时刻
                for i in 0..50 {
                    let key = format!("concurrent_key_{}", i);
                    env.set(&key, &format!("task{}_val{}", t, i));
                    // 此处无 await 在锁内
                    let _ = env.get(&key);
                }
            }));
        ...
        assert!(
            result.is_ok(),
            "P3-CONC-03: 任务饥饿（锁可能跨 await 持有）"
        );
    }

    /// P3-CONC-04：Env 内存边界 — 大量 key 写入不 OOM
    ///
    /// 验证 Env 在写入大量 key 时内存增长可控，
    /// 不会因 HashMap 扩容策略导致内存尖峰。
    #[tokio::test]
    async fn test_env_memory_bounded_under_load() {
        let env = Env::default();
        let num_keys = 10_000;

        // 写入大量 key
        for i in 0..num_keys {
            env.set(&format!("stress_key_{}", i), &"x".repeat(100));
        }

        // 验证所有 key 可读
        assert_eq!(env.get("stress_key_0"), Some("x".repeat(100)));
        assert_eq!(env.get("stress_key_9999"), Some("x".repeat(100)));

        // 删除一半（触发 HashMap 缩容路径）
        for i in 0..num_keys / 2 {
            env.remove(&format!("stress_key_{}", i));
        }

        // 验证删除后仍可正常操作
        assert_eq!(env.get("stress_key_0"), None);
        assert_eq!(env.get("stress_key_9999"), Some("x".repeat(100)));

        // 重新写入被删除的 key（触发 HashMap rehash）
        for i in 0..num_keys / 2 {
            env.set(&format!("stress_key_{}", i), &"y".repeat(50));
        }
        assert_eq!(env.get("stress_key_0"), Some("y".repeat(50)));
    }
}
