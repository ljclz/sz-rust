...
///
/// ```ignore
/// use sz_rust_cache_facade::{default_cache, init_default_cache, MemoryCacheDriver};
///
...
///
/// ## 与 `sz_rust_orm_facade::Cache` 的关系
///
/// `sz_rust_orm_facade::Cache` trait 提供底层 KV 操作（`get/set/delete/exists`），
...
///
/// 基于 `sz_rust_orm_facade::MemoryCache`，提供进程内内存缓存。
///
/// ## PHP 对齐
///
...
impl Cache {
    ...
    ///
    /// ```ignore
    /// # async fn example(cache: &sz_rust_cache_facade::Cache) {
...
// ============================================================================
// MultiLevelCacheDriver — 多级缓存驱动（复用 sz_rust_orm_facade::MultiLevelCache）
// ============================================================================
...
mod tests {
    ...
    // ========================================================================
    // 混沌工程测试（P3-CHAOS-01 ~ P3-CHAOS-03）
    // ========================================================================
    // 注意：fetch_singleflight 是同步 API，Cache 不实现 Clone。
    // 并发测试统一使用 std::thread + Arc 模式（与 test_fetch_singleflight_concurrent_only_one_fetcher_call 一致）。

    /// P3-CHAOS-01：缓存雪崩防护 — singleflight 合并并发请求
    ///
    /// 模拟 50 个并发请求同时请求同一个未缓存的 key。
    /// 预期：singleflight 将 50 个请求合并为 1 次实际回源，
    /// 其余 49 个请求等待同一结果，不触发缓存击穿。
    #[test]
    ...
    /// P3-CHAOS-02：缓存降级 — 慢回源超时场景
    ///
    /// fetch_singleflight 是同步阻塞 API。验证：
    /// 1. 慢回源在阈值时间内不会完成（调用方需自行用 spawn_blocking + timeout 包裹）
    /// 2. 慢回源最终完成后结果正确传播（使用 mpsc + recv_timeout 模拟降级判定）
    #[test]
        ...
        for w in 0..num_writers {
            ...
            handles.push(std::thread::spawn(move || {
                ...
                for i in 0..writes_per_writer {
                    let key = format!("ttl_key_{}", i % 20); // 20 个 key 循环写入
                    let val = format!("writer{}_val{}", w, i);
