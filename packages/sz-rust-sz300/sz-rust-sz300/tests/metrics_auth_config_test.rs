...
fn test_metrics_auth_from_env_default() {
    std::env::remove_var("SZ300_METRICS_ALLOWED_IPS");
    std::env::remove_var("SZ300_METRICS_BEARER_TOKEN");
    std::env::remove_var("SZ300_METRICS_AUTH_ENABLED");
...
fn test_metrics_auth_from_env_with_token() {
    std::env::set_var("SZ300_METRICS_BEARER_TOKEN", "my-secret-token");
    let config = MetricsAuthConfig::from_env();
...
fn test_metrics_auth_from_env_with_ips() {
    std::env::set_var("SZ300_METRICS_ALLOWED_IPS", "10.0.0.1,10.0.0.2,192.168.1.0/24");
    let config = MetricsAuthConfig::from_env();
