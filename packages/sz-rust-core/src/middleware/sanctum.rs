...
use crate::error::BaseException;
use crate::middleware::auth::{base_exception_to_response, extract_route_uri, is_route_allowed};
...
pub async fn sanctum_middleware(
    ...
    let token = match auth_header {
        Some(value) => {
            ...
            crate::middleware::auth::extract_token_from_header(raw)
        }
        None => None,
    };
...
mod tests {
    ...
    fn test_generate_random_hex_uniqueness() {
        ...
        let values: Vec<String> = (0..10)
            .map(|_| generate_random_hex().unwrap())
            .collect();
