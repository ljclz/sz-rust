...
//! | 模块 | 内容 | 实现阶段 |
//! |------|------|---------|
//! | [`has_many`] | `HasMany` 关联 PHP 命名约定 + SQL 片段构造器 | ✅ |
//! | [`belongs_to`] | `BelongsTo` 关联 PHP 命名约定 + SQL 片段构造器 | ✅ |
//! | [`has_one`] | `HasOne` 关联 PHP 命名约定 + SQL 片段构造器 | ✅ |
//! | [`belongs_to_many`] | `BelongsToMany` 关联 PHP 命名约定 + SQL 片段构造器 | ✅ |
//! | [`with`] | 关联预加载（批量 IN 查询 + 数据处理纯函数 + 语法解析） | ✅ |
//! | [`find_with_related`] | find_with_related API（JOIN / eager_sql / 子查询 三种模式） | ✅ |
//! | [`cache`] | 关联缓存（对齐 PHP `withCache()` / `Cache::clear($tag)`） | ✅ |
//! | [`morph`] | MorphTo/MorphMany 多态关联（对齐 PHP `morphMany()` / `morphTo()`） | ✅ |
//! | [`n_plus_one`] | N+1 问题检测（SQL 计数 + 模板分组 + 告警生成） | ✅ |
//!
//! ## PHP 端关联关系对齐
//!
...
//! 4. **批量预加载辅助**：`has_many_in_sql` / `has_one_in_sql` /
...
//! 5. **find_with_related API**：re-export sz-orm-core
...
//! 7. **多态关联**：re-export sz-orm-core `MorphMany` / `MorphTo`，
...
//! 8. **N+1 问题检测**：作为 PHP 端的扩展（PHP 端不主动检测 N+1
...
pub use sz_rust_orm_facade::find_with_related::WithRelation;
pub use sz_rust_orm_facade::{
