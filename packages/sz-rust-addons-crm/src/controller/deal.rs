...
impl DealController {
    pub async fn list<R: Repository<Deal, Key = OrmValue>>(
        repo: &R, page: u64, page_size: u64, stage: Option<String>,
    ) -> Value {
        let conditions: Vec<WhereCondition> = if let Some(s) = stage.as_deref() {
            if s.is_empty() { Vec::new() }
            else { vec![WhereCondition::new("stage", WhereOp::Eq, OrmValue::String(s.to_string()))] }
        } else { Vec::new() };
    ...
    pub async fn update<R: Repository<Deal, Key = OrmValue>>(repo: &R, id: i64, body: Value) -> Value {
            ...
            macro_rules! patch {
                ($field:ident, str) => {
                    if let Some(v) = obj.get(stringify!($field)) {
                        if let Some(s) = v.as_str() { deal.$field = s.to_string(); }
                    }
                };
                ($field:ident, f64) => {
                    if let Some(v) = obj.get(stringify!($field)) {
                        if let Some(n) = v.as_f64() { deal.$field = n; }
                    }
                };
                ($field:ident, i64) => {
                    if let Some(v) = obj.get(stringify!($field)) {
                        if let Some(n) = v.as_i64() { deal.$field = n; }
                    }
                };
                ($field:ident, u8) => {
                    if let Some(v) = obj.get(stringify!($field)) {
                        if let Some(n) = v.as_u64() { deal.$field = n as u8; }
                    }
                };
            }
            patch!(name, str);
    ...
    pub async fn pipeline<R: Repository<Deal, Key = OrmValue>>(repo: &R) -> Value {
        let stages = ["prospect", "qualified", "proposal", "negotiation", "closed_won", "closed_lost"];
