...
use md5::{Digest, Md5};
use rand::Rng;
use sz_rust_orm_facade::Storage;
...
impl AliyunStorageEngine {
    ...
    /// 创建底层存储
    fn create_storage(&self) -> sz_rust_orm_facade::AliyunOssStorage {
        sz_rust_orm_facade::AliyunOssStorage::new(
...
impl QcloudStorageEngine {
    ...
    /// 创建底层存储
    fn create_storage(&self) -> sz_rust_orm_facade::TencentCosStorage {
        sz_rust_orm_facade::TencentCosStorage::new(
            self.config.bucket.clone(),
...
impl QiniuStorageEngine {
    ...
    /// 创建底层存储
    fn create_storage(&self) -> sz_rust_orm_facade::QiniuKodoStorage {
        sz_rust_orm_facade::QiniuKodoStorage::new(self.config.bucket.clone())
    }
}
...
impl S3StorageEngine {
    ...
    /// 创建底层存储
    fn create_storage(&self) -> sz_rust_orm_facade::S3Storage {
        sz_rust_orm_facade::S3Storage::new(self.config.bucket.clone(), self.config.region.clone())
    }
}
