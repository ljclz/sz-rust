...
//!
//! ```ignore
//! use sz_rust_infra_facade::static_files::{static_router, static_router_spa, serve_file};
//! use axum::Router;
//!
//! // 普通静态目录（基于 ServeDir）
...
/// # 示例
/// ```
/// # use sz_rust_infra_facade::static_files::extract_version_hash;
/// assert_eq!(
...
mod tests {
    ...
    /// P8-PATH-01：`is_path_safe` 拒绝 `../` 路径穿越
    #[test]
    fn test_is_path_safe_rejects_parent_traversal() {
        let root = tempfile::tempdir().unwrap();
        let child = root.path().join("public");
        std::fs::create_dir(&child).unwrap();

        // 尝试通过 ../ 逃逸到 root 的父目录
        let escape = child.join("../../escape.txt");
        assert!(
            !is_path_safe(&escape, &child),
            "P8-PATH-01: ../ 路径穿越应被拒绝"
        );
    }

    /// P8-PATH-02：`is_path_safe` 允许根目录内的合法路径
    #[test]
    fn test_is_path_safe_allows_valid_path() {
        let root = tempfile::tempdir().unwrap();
        let file = root.path().join("index.html");
        std::fs::write(&file, "hello").unwrap();

        assert!(
            is_path_safe(&file, root.path()),
            "P8-PATH-02: 根目录内的合法路径应被允许"
        );
    }

    /// P8-PATH-03：`is_path_safe` 拒绝绝对路径逃逸
    #[test]
    fn test_is_path_safe_rejects_absolute_path() {
        let root = tempfile::tempdir().unwrap();
        let child = root.path().join("public");
        std::fs::create_dir(&child).unwrap();

        // 绝对路径指向根目录外
        let absolute = std::path::PathBuf::from(if cfg!(windows) {
            r"C:\etc\passwd"
        } else {
            "/etc/passwd"
        });
        assert!(
            !is_path_safe(&absolute, &child),
            "P8-PATH-03: 绝对路径逃逸应被拒绝"
        );
    }

    /// P8-PATH-04：`has_traversal_component` 检测 `..` 组件
    #[test]
    fn test_has_traversal_component_detects_parent_dir() {
        let path = std::path::PathBuf::from("public/../../secret.txt");
        assert!(
            has_traversal_component(&path),
            "P8-PATH-04: 包含 .. 组件的路径应被检测"
        );
    }

    /// P8-PATH-05：`has_traversal_component` 允许不含 `..` 的路径
    #[test]
    fn test_has_traversal_component_allows_safe_path() {
        let path = std::path::PathBuf::from("public/assets/style.css");
        assert!(
            !has_traversal_component(&path),
            "P8-PATH-05: 不含 .. 的合法路径不应被标记"
        );
    }
}
