...
//! ## 设计目标
//!
//! 业务包通过 `sz_rust_orm_facade::grpc::*` 访问 gRPC 功能，
//! 而非直接依赖 `sz-orm-grpc`，保持 facade 收口。
//!
//! ## 启用方式
//!
//! 在 `sz-rust-orm-facade` 的 Cargo.toml 中启用 `grpc` feature：
//! ```toml
//! sz-rust-orm-facade = { version = "0.3", features = ["grpc"] }
//! ```
//!
//! ## 核心类型
//!
...
//!
//! ```rust,ignore
//! use sz_rust_orm_facade::grpc::{GrpcServer, GrpcServiceDef, GrpcMethod, InMemoryUserService};
//!
...
compile_error!(
    "gRPC facade requires the `grpc` feature. \
     Enable it: sz-rust-orm-facade = { features = [\"grpc\"] }"
);
