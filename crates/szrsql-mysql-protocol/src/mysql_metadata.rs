...
fn rct(name: &str) -> ResultColumn {
    ResultColumn { name: name.to_string(), column_type: ColumnType::Text }
}
fn rci(name: &str) -> ResultColumn {
    ResultColumn { name: name.to_string(), column_type: ColumnType::Int64 }
}
...
async fn handle_show_table_status(
    ...
    let columns = vec![
        rct("Name"), rct("Engine"), rci("Version"), rct("Row_format"),
        rci("Rows"), rci("Avg_row_length"), rci("Data_length"),
        rci("Max_data_length"), rci("Index_length"), rci("Data_free"),
        rct("Auto_increment"), rct("Create_time"), rct("Update_time"),
        rct("Check_time"), rct("Collation"), rct("Checksum"),
        rct("Create_options"), rct("Comment"),
    ];

    let mut rows = Vec::new();
    for (table_name, _cols) in &tables {
        if let Some(ref pattern) = like_pattern {
            if !mysql_like_match(table_name, pattern) { continue; }
        }
        ...
        rows.push(vec![
            ...
            Value::Int64(0), Value::Int64(16384), Value::Int64(0),
            Value::Int64(0), Value::Int64(0), Value::Null,
            Value::Null, Value::Null, Value::Null,
            Value::Text("utf8mb4_general_ci".to_string()),
            Value::Null, Value::Text("".to_string()), Value::Text("".to_string()),
        ]);
    }
    Some((columns, rows))
}
...
async fn handle_show_create_table(
    ...
    let upper = sql.to_uppercase();
    let after = sql[upper.find("SHOW CREATE TABLE ")? + 18..].trim().trim_end_matches(';').trim();
    let (db_opt, table_name) = parse_qualified_table_name(after);
    let (_actual_db, actual_table, columns) = find_table_columns(shared, db_opt.as_deref(), &table_name).await?;
    ...
    for col in &columns {
        ...
        if col.not_null { def.push_str(" NOT NULL"); }
        if col.primary_key { primary_keys.push(col.name.clone()); }
        col_defs.push(def);
    }
    ...
    let ddl = format!("CREATE TABLE `{}` (\n{}\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4", actual_table, col_defs.join(",\n"));

    Some((vec![rct("Table"), rct("Create Table")], vec![vec![Value::Text(actual_table), Value::Text(ddl)]]))
}
...
async fn handle_show_columns(
    ...
    let after_from = if is_full { sql[23..].trim() } else if upper.starts_with("SHOW COLUMNS FROM ") { sql[18..].trim() } else { sql[17..].trim() };
    let table_part = after_from.split_whitespace().next().unwrap_or("").trim_end_matches(';');
    let (db_opt, table_name) = parse_qualified_table_name(table_part);
    let db = db_opt.or_else(|| current_db.clone());
    let (_, _actual_table, columns) = find_table_columns(shared, db.as_deref(), &table_name).await?;

    let columns_result = if is_full {
        vec![rct("Field"), rct("Type"), rct("Collation"), rct("Null"), rct("Key"), rct("Default"), rct("Extra"), rct("Privileges"), rct("Comment")]
    } else {
        vec![rct("Field"), rct("Type"), rct("Null"), rct("Key"), rct("Default"), rct("Extra")]
    };
    ...
    for col in &columns {
        ...
        let default_val = col.default.as_ref().map(|_| Value::Text("NULL".to_string())).unwrap_or(Value::Null);
        let extra = if col.primary_key { "auto_increment".to_string() } else { String::new() };
        if is_full {
            rows.push(vec![Value::Text(col.name.clone()), Value::Text(mysql_type), Value::Text("utf8mb4_general_ci".to_string()), Value::Text(null_str.to_string()), Value::Text(key_str.to_string()), default_val, Value::Text(extra), Value::Text("select,insert,update,references".to_string()), Value::Text("".to_string())]);
        } else {
            rows.push(vec![Value::Text(col.name.clone()), Value::Text(mysql_type), Value::Text(null_str.to_string()), Value::Text(key_str.to_string()), default_val, Value::Text(extra)]);
        }
    }
...
async fn handle_show_index(
    ...
    let (db_opt, table_name) = parse_qualified_table_name(table_part);
    let db = db_opt.or_else(|| current_db.clone());
    let (actual_db, actual_table, columns) = find_table_columns(shared, db.as_deref(), &table_name).await?;

    let result_columns = vec![
        rct("Table"), rci("Non_unique"), rct("Key_name"), rci("Seq_in_index"),
        rct("Column_name"), rct("Collation"), rci("Cardinality"), rct("Sub_part"),
        rct("Packed"), rct("Null"), rct("Index_type"), rct("Comment"),
        rct("Index_comment"), rct("Visible"), rct("Expression"),
    ];
    ...
    if !pk_cols.is_empty() {
        for (seq, col) in pk_cols.iter().enumerate() {
            let null_str = if col.not_null { "" } else { "YES" };
            rows.push(vec![
...
async fn handle_show_tables(
    ...
    let filter_views = if upper.contains("TABLE_TYPE != 'VIEW'") || upper.contains("TABLE_TYPE!='VIEW'") {
        Some(false)
    ...
    let columns = if is_full { vec![rct(&col_name), rct("Table_type")] } else { vec![rct(&col_name)] };
    ...
    for (table_name, _) in &tables {
        match filter_views {
            Some(true) => continue,
            Some(false) => {
                if is_full { rows.push(vec![Value::Text(table_name.clone()), Value::Text("BASE TABLE".to_string())]); }
                else { rows.push(vec![Value::Text(table_name.clone())]); }
            }
            None => {
                if is_full { rows.push(vec![Value::Text(table_name.clone()), Value::Text("BASE TABLE".to_string())]); }
                else { rows.push(vec![Value::Text(table_name.clone())]); }
...
async fn handle_information_schema(
    ...
    if upper.contains("INFORMATION_SCHEMA.TABLES") {
        if upper.contains("COUNT(*)") { return handle_info_schema_tables_count(sql, shared).await; }
    ...
    if upper.contains("INFORMATION_SCHEMA.ROUTINES") {
        return Some((vec![rct("ROUTINE_SCHEMA"), rct("ROUTINE_NAME"), rct("PARAMETER")], Vec::new()));
    }
    if upper.contains("INFORMATION_SCHEMA.VIEWS") {
        return Some((vec![rct("TABLE_NAME"), rct("CHECK_OPTION"), rct("IS_UPDATABLE"), rct("SECURITY_TYPE"), rct("DEFINER")], Vec::new()));
    }
    if upper.contains("INFORMATION_SCHEMA.PARAMETERS") {
        return Some((vec![rct("PARAMETER_NAME")], Vec::new()));
    }
    if upper.contains("INFORMATION_SCHEMA.KEY_COLUMN_USAGE") || upper.contains("INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS") || upper.contains("INFORMATION_SCHEMA.TABLE_CONSTRAINTS") {
        return Some((vec![rct("CONSTRAINT_NAME"), rct("TABLE_SCHEMA"), rct("TABLE_NAME"), rct("COLUMN_NAME")], Vec::new()));
    }
    if upper.contains("INFORMATION_SCHEMA.STATISTICS") {
        return Some((vec![rct("TABLE_SCHEMA"), rct("TABLE_NAME"), rci("NON_UNIQUE"), rct("INDEX_SCHEMA"), rct("INDEX_NAME"), rci("SEQ_IN_INDEX"), rct("COLUMN_NAME"), rct("COLLATION"), rci("CARDINALITY"), rct("SUB_PART"), rct("PACKED"), rct("NULLABLE"), rct("INDEX_TYPE"), rct("COMMENT"), rct("INDEX_COMMENT")], Vec::new()));
    }
    if upper.contains("INFORMATION_SCHEMA.ENGINES") {
        if upper.contains("COUNT") { return Some((vec![rct("support_ndb")], vec![vec![Value::Int64(0)]])); }
        return Some((vec![rct("ENGINE"), rct("SUPPORT"), rct("COMMENT"), rct("TRANSACTIONS"), rct("XA"), rct("SAVEPOINTS")], vec![vec![Value::Text("InnoDB".to_string()), Value::Text("YES".to_string()), Value::Text("Supports transactions, row-level locking, and foreign keys".to_string()), Value::Text("YES".to_string()), Value::Text("YES".to_string()), Value::Text("YES".to_string())]]));
    }
    if upper.contains("INFORMATION_SCHEMA.EVENTS") {
        return Some((vec![rct("EVENT_CATALOG"), rct("EVENT_SCHEMA"), rct("EVENT_NAME"), rct("DEFINER"), rct("TIME_ZONE"), rct("EVENT_DEFINITION")], Vec::new()));
    }
...
async fn handle_info_schema_schemata(
    ...
    let mut dbs: Vec<String> = all_tables.iter().map(|(db, _, _, _)| db.clone()).collect::<std::collections::HashSet<_>>().into_iter().collect();
    dbs.sort();
    let columns = vec![rct("SCHEMA_NAME"), rct("DEFAULT_CHARACTER_SET_NAME"), rct("DEFAULT_COLLATION_NAME")];
    let mut rows = Vec::new();
    for db in &dbs {
        rows.push(vec![Value::Text(db.clone()), Value::Text("utf8mb4".to_string()), Value::Text("utf8mb4_general_ci".to_string())]);
    }
    if !dbs.iter().any(|d| d.eq_ignore_ascii_case("information_schema")) {
        rows.push(vec![Value::Text("information_schema".to_string()), Value::Text("utf8".to_string()), Value::Text("utf8_general_ci".to_string())]);
    }
    Some((columns, rows))
}
...
async fn handle_info_schema_tables(
    ...
    for (db, table_name, _, _) in &all_tables {
        if let Some(ref filter) = schema_filter {
            if !db.eq_ignore_ascii_case(filter) { continue; }
        }
        rows.push(vec![Value::Text(db.clone()), Value::Text(table_name.clone()), Value::Text("BASE TABLE".to_string())]);
    }
    rows.sort_by(|a, b| (value_as_str(&a[0]), value_as_str(&a[1])).cmp(&(value_as_str(&b[0]), value_as_str(&b[1]))));
    Some((columns, rows))
}
...
async fn handle_info_schema_tables_count(
    ...
    let table_count = if let Some(ref filter) = schema_filter {
        all_tables.iter().filter(|(db, _, _, _)| db.eq_ignore_ascii_case(filter)).count()
    } else { all_tables.len() };
    let has_routines_union = upper.contains("ROUTINES");
    let columns = vec![rct("COUNT(*)")];
    let mut rows = Vec::new();
    rows.push(vec![Value::Int64(table_count as i64)]);
    if has_routines_union { rows.push(vec![Value::Int64(0)]); }
    Some((columns, rows))
}
...
async fn handle_info_schema_columns(
    ...
    let columns = vec![rct("TABLE_SCHEMA"), rct("TABLE_NAME"), rct("COLUMN_NAME"), rct("COLUMN_TYPE")];
    let all_tables = get_all_tables(shared).await;
    let mut rows = Vec::new();
    for (db, table_name, _, cols) in &all_tables {
        if !schemas.is_empty() {
            if !schemas.iter().any(|s| db.eq_ignore_ascii_case(s)) { continue; }
        }
        for col in cols {
            let mysql_type = column_type_to_mysql(&col.data_type);
            rows.push(vec![Value::Text(db.clone()), Value::Text(table_name.clone()), Value::Text(col.name.clone()), Value::Text(mysql_type)]);
        }
    }
    rows.sort_by(|a, b| (value_as_str(&a[0]), value_as_str(&a[1]), value_as_str(&a[2])).cmp(&(value_as_str(&b[0]), value_as_str(&b[1]), value_as_str(&b[2]))));
    Some((columns, rows))
}
    ...
    match pattern[pi] {
        '%' => {
            for skip in 0..=(text.len() - ti) {
                if like_match_impl(text, pattern, ti + skip, pi + 1) { return true; }
            }
            false
        }
        '_' => { if ti < text.len() { like_match_impl(text, pattern, ti + 1, pi + 1) } else { false } }
        c => { if ti < text.len() && text[ti].to_ascii_lowercase() == c.to_ascii_lowercase() { like_match_impl(text, pattern, ti + 1, pi + 1) } else { false } }
    }
}
