...
fn defect_02_null_key_index_consistency_basic() {
    ...
    // 插入空 key
    bt.insert(Vec::new(), vec![42u8]).expect("insert empty key");
    ...
    // 插入非空 key 混合
    for v in 1..=100i64 {
        bt.insert(encode_i64_key(v), vec![v as u8]).expect("insert");
    }
    ...
    assert_eq!(pairs[0].1, vec![42u8], "empty key tuple_id mismatch");
...
fn defect_02_null_key_index_consistency_upsert_and_split() {
    ...
    for tid in 1..=10u32 {
        bt.insert(Vec::new(), vec![tid as u8])
            .expect("insert empty key");
    }
    assert_eq!(
        bt.search(&[]).expect("search"),
        Some(vec![10u8]),
    ...
    assert_eq!(
        pairs[0].1,
        vec![10u8],
        "empty key tuple_id mismatch after split"
    );
...
fn defect_03_unique_index_duplication_basic() {
    ...
    for tid in 1..=100u32 {
        bt.insert(key.clone(), vec![tid as u8]).expect("insert");
    }
    ...
    bt.insert(key.clone(), vec![231u8]).expect("upsert");
    assert_eq!(
        bt.search(&key).expect("search"),
        Some(vec![231u8]),
    ...
    for tid in 0..threads {
        ...
        handles.push(thread::spawn(move || {
            for i in 0..per_thread {
                ...
                let mut guard = bt.lock();
                guard
                    .insert(key_clone.clone(), vec![tuple_id as u8])
                    .expect("insert");
            }
        }));
    }
...
fn defect_04_stale_separator_search_all_deleted() {
    ...
    for v in [1, 25, 50, 75, 100].iter().cloned() {
        assert_eq!(
            bt.search(&encode_i64_key(v)).expect("search"),
            None,
    ...
    // 重新插入应正常工作
    for v in 1..=50i64 {
        bt.insert(encode_i64_key(v), vec![v as u8])
            .expect("reinsert");
    }
    for v in 1..=50i64 {
        assert_eq!(
...
fn defect_05_concurrent_mixed_ops_invariants() {
    ...
    // 预填充 1000 个 key
    {
        ...
        for v in 0..1000i64 {
            guard
                .insert(encode_i64_key(v), vec![v as u8])
                .expect("prefill");
        }
    }
            ...
            for _ in 0..ops_per_thread {
                ...
                match op {
                    0 => {
                        // insert
                        guard
                            .insert(encode_i64_key(v), vec![(v % 65536) as u8])
                            .expect("insert");
...
fn defect_05_concurrent_mixed_ops_insert_delete_same_keys() {
        ...
        for round in 0..3 {
            for v in 0..key_count {
                bt_a.lock()
                    .insert(
                        encode_i64_key(v),
                        vec![((round * key_count + v) % 65536) as u8],
                    )
                    .expect("insert");
            }
        }
    ...
    let h_b = thread::spawn(move || {
        for _ in 0..3 {
            for v in 0..key_count {
                let _ = bt_b.lock().delete(&encode_i64_key(v));
            }
        }
    });
    ...
    let h_c = thread::spawn(move || {
        for _ in 0..3 {
            for v in 0..key_count {
                let _ = bt_c.lock().search(&encode_i64_key(v));
            }
        }
    });
...
fn defect_06_bulk_load_equivalence_basic() {
    ...
    let items: Vec<(Vec<u8>, Vec<u8>)> = keys
        .iter()
        .map(|&v| (encode_i64_key(v), vec![v as u8]))
        .collect();

    // 树 A: 逐条 insert
    let mut bt_a = BTree::with_default_order();
    for (k, tid) in &items {
        bt_a.insert(k.clone(), tid.clone()).expect("insert");
    }
...
fn defect_06_bulk_load_equivalence_edge_cases() {
    ...
    let single = vec![(encode_i64_key(42), vec![42u8])];
    ...
    let initial: Vec<(Vec<u8>, Vec<u8>)> = (1..=100)
        .map(|v| (encode_i64_key(v), vec![v as u8]))
        .collect();
    ...
    let unsorted = vec![
        (encode_i64_key(10), vec![10u8]),
        (encode_i64_key(5), vec![5u8]), // 乱序
    ];
...
fn m1_milestone_all_defects_summary_large_scale() {
    ...
    for (idx, &v) in keys.iter().enumerate() {
        bt.insert(encode_i64_key(v), vec![(idx % 65536) as u8])
            .expect("insert");
    }
